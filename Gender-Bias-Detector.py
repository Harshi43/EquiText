...
def extract_ncert_text(pdf_files):
    ...
    exclude_patterns = [
        r'^(\d+\s*$|Chapter|©\s*NCERT|Fig\.|Table)',  # Page numbers, headers
...
def plot_gender_frequency(gender_analysis):
    """
    Plot frequency of male vs. female terms.
    """
    for file, counts in gender_analysis.items():
        genders = list(counts.keys())
        values = list(counts.values())

        plt.bar(genders, values, color=['blue', 'pink'])
        plt.title(f"Gendered Word Frequency in {file}")
        plt.xlabel("Gender")
        plt.ylabel("Frequency")
        plt.show()

def plot_roles_by_gender(roles_analysis):
    """
    Plot roles assigned to genders as bar charts.
    """
    for file, roles in roles_analysis.items():
        male_roles = len(set(roles["male"]))
        female_roles = len(set(roles["female"]))

        plt.bar(["Male Roles", "Female Roles"], [male_roles, female_roles], color=['blue', 'pink'])
        plt.title(f"Roles Distribution in {file}")
        plt.xlabel("Gender")
        plt.ylabel("Number of Unique Roles")
        plt.show()

# Call the functions
plot_gender_frequency(gender_analysis)
plot_roles_by_gender(roles_analysis)

"""Generate a Report"""

import pandas as pd

def generate_report(gender_analysis, roles_analysis, sentiment_analysis):
    """
    Combine analysis into a CSV report.
    """
    report = []

    for file in gender_analysis.keys():
        male_count = gender_analysis[file]["male"]
        female_count = gender_analysis[file]["female"]
        male_roles = ", ".join(set(roles_analysis[file]["male"]))
        female_roles = ", ".join(set(roles_analysis[file]["female"]))
        male_sentiments = sentiment_analysis[file]["male"]
        female_sentiments = sentiment_analysis[file]["female"]

        report.append({
            "File": file,
            "Male Word Count": male_count,
            "Female Word Count": female_count,
            "Male Roles": male_roles,
            "Female Roles": female_roles,
            "Male Sentiments": male_sentiments,
            "Female Sentiments": female_sentiments
        })

    df = pd.DataFrame(report)
    df.to_csv("gender_bias_report.csv", index=False)
    print("Report generated: gender_bias_report.csv")

# Generate the report
generate_report(gender_analysis, roles_analysis, sentiment_analysis)

"""Code: Compute Bias Score"""

def calculate_bias_score(gender_analysis, roles_analysis, sentiment_analysis):
    """
    Calculate a bias score for each textbook file.
    """
    bias_scores = {}

    for file in gender_analysis.keys():
        # Visibility: Ratio of male to female terms
        male_count = gender_analysis[file]["male"]
        female_count = gender_analysis[file]["female"]
        visibility_score = (male_count - female_count) / (male_count + female_count + 1)  # Normalize

        # Roles: Diversity of roles (unique counts)
        male_roles = len(set(roles_analysis[file]["male"]))
        female_roles = len(set(roles_analysis[file]["female"]))
        role_score = (male_roles - female_roles) / (male_roles + female_roles + 1)

        # Sentiment: Ratio of positive to negative sentences for each gender
        male_sentiments = sentiment_analysis[file]["male"]
        female_sentiments = sentiment_analysis[file]["female"]
        ...
        male_sentiment_score = sentiment_ratio(male_sentiments)
        female_sentiment_score = sentiment_ratio(female_sentiments)

        sentiment_score = (male_sentiment_score - female_sentiment_score) / 2  # Normalize

        # Final Bias Score: Weighted sum
        bias_score = visibility_score * 0.4 + role_score * 0.3 + sentiment_score * 0.3
        bias_scores[file] = round(bias_score, 2)

    return bias_scores

# Calculate bias scores
bias_scores = calculate_bias_score(gender_analysis, roles_analysis, sentiment_analysis)

# Display results
for file, score in bias_scores.items():
    print(f"Bias Score for {file}: {score}")